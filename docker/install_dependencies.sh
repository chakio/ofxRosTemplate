...
if [ $(expr $MAJOR_VERSION \< 13 ) -eq 1 ]; then
    ...
    sudo update-alternatives --install -y /usr/bin/gcc gcc /usr/bin/gcc-4.6 20
    sudo update-alternatives --install -y /usr/bin/gcc gcc /usr/bin/gcc${CXX_VER} 50
    sudo update-alternatives --install -y /usr/bin/g++ g++ /usr/bin/g++-4.6 20
    sudo update-alternatives --install -y /usr/bin/g++ g++ /usr/bin/g++${CXX_VER} 50
fi
